...
const MongoStore = require('connect-mongo'); // salvar as sessões na base de dados, por padrão é salvo na memória do servidor, mas isso consome muito recurso
const flash = require('connect-flash');
//###########################

const routes = require('./routes.js'); //rotas: / (página inicial), /contato, /sobre, etc..
const path = require('path');

//################ segurança
const helmet = require('helmet');// professor não deu detalhes, disse para pesquisar, mas ele faz algo relacionado à segurança nos cabeçalhos das páginas
const csrtf = require('csurf'); // ver middleware na pasta middlewares, são tokens de segurança
//###########################
...
app.use(express.json()); // faz o mesmo que a linha acima mas com json

// esta é uma pasta aonde vai ficar meus conteudos estaticos. Ela não é uma rota, não preciso digitar /public como exemplo vou colocar um arquivo txt nela e no navegador acesso ele direto, assim: 127.0.0.1:3000/teste.txt
...
// o primeiro 'views' é o nome do recurso que eu quero usar, no caso, as views o src e views é o caminho de onde estão as minhas views e que estou deixando para o path.reesolve resolver p mim. 
app.set('views', path.resolve(__dirname, 'src', 'views'));

// view engine: mesmo coisa: primeira string é o nome do recurso, e o segundo parametro é a engine que eu quero usar para reinderizar as views, tem vários, aqui vamos usar o ejs. E necessário instalar o ejs: terminal: npm i ejs
app.set('view engine', 'ejs');

// importante que esteja aqui, se colocar mais p cima não funciona
app.use(csrtf());

// midlle global. Descobri da pior forma que este use de Midlleware tem que vir DEPOIS da linha acima que é a que permite usar o body nos parametros req e res "app.use(express.urlencoded({extended: true}))""
