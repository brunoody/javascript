...
class UserController {
  ...
  async store(req, res) {
    try {
      const novoUser = await _User2.default.create(req.body); // vem lá o insomnia escolhendo a opção BODY > JSON e digitando um json
  ...
  */
  ...
  async update(req, res) {
    try {
      const user = await _User2.default.findByPk(req.userId); // esse userId vem do toquem, que foi extraíno no middleware loginRequired
  ...
  async delete(req, res) {
    try {
      const user = await _User2.default.findByPk(req.userId); // esse userId vem do toquem, que foi extraíno no middleware
